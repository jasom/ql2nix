...
{ buildLispPackage, stdenv, fetchurl, lisp-project_persistent-tables, 
   lisp_lisp-unit, lisp_random-access-lists,  
  sbcl, clisp, ccl,  
...
in
  buildLispPackage {
      propagatedBuildInputs = [ lisp_lisp-unit lisp_random-access-lists  ];
      inherit stdenv;
      systemName = "persistent-tables";
      
      sourceProject = "${lisp-project_persistent-tables}";
      patches = [];
      lisp_dependencies = "${lisp_lisp-unit} ${lisp_random-access-lists}";
