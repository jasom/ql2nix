...
{ buildLispPackage, stdenv, fetchurl, lisp-project_immutable-struct, 
   lisp_alexandria, lisp_closer-mop, lisp_trivia,  
  ccl, clisp, sbcl,  
...
in
  buildLispPackage {
      propagatedBuildInputs = [ lisp_alexandria lisp_closer-mop lisp_trivia  ];
      inherit stdenv;
      systemName = "immutable-struct";
      
      sourceProject = "${lisp-project_immutable-struct}";
      patches = [];
      lisp_dependencies = "${lisp_alexandria} ${lisp_closer-mop} ${lisp_trivia}";
